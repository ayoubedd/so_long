...
int	hooks_handler(int keycode, t_game *game)
{
	if (keycode == KB_MOVE_BACKWARD)
		...
		cleanup(game);
	if ((keycode == KB_MOVE_RIGHT
			|| keycode == KB_MOVE_LEFT
			|| keycode == KB_MOVE_BACKWARD
			|| keycode == KB_MOVE_FORWARD) && game->player.moves > 0)
		ft_printf("\rThe player made %d moves so far.", game->player.moves);
	return (0);
}
